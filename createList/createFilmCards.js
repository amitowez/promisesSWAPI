...
export function createFilmCards(films){
    const filmContainer = createList() 
    films.forEach(film => {
        const cardFilm = document.createElement('div')
        cardFilm.classList.add('card-mini')
        cardFilm.style.cursor = 'pointer'
        filmField.forEach(field =>{
            const line = document.createElement('div')
            line.classList.add('cenried')
            console.log()
            line.innerHTML = `
                <p>${field.name || field}:</p> <p>${typeof field =='string' ? film[field] : film[field.name].length}</p>
            `
            cardFilm.appendChild(line)
        })
        filmContainer.appendChild(cardFilm)
    });
...
// vehicles: (4) ['https://swapi.dev/api/vehicles/4/', 'https://swapi.dev/api/vehicles/6/', 'https://swapi.de